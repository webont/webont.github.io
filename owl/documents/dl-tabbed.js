...
if (document.getElementsByTagName) document.write("<style type='text/css'>dl.tabbed dd {display: none;}</style>");
 ...
 makeLinks : function(dl){
  ...
  for (var i = 0; i < el.length; i++) {
   if (!el[i].getElementsByTagName("A")[0]){
    el[i].innerHTML = "<a " + tabbedDL.event + "='tabbedDL.showDD(this);' href='javascript:;'>" + el[i].innerHTML + "</a>";
    el[i].className = "";
    }
   }
 },
